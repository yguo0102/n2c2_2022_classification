...
def main():
    ...
    def compute_metrics(p: EvalPrediction) -> Dict:
        ...
        metrics = {
            ...
            'f1_pmabuse' : classification_report(y_true=labels, y_pred=preds, output_dict=True)['0']['f1-score'] if ex_args['metric'] == 'f1_pmabuse' else 0,
        }
        return metrics
    ...
    if training_args.do_predict:
        logging.info("*** Test ***")
        test_datasets = [test_dataset, eval_dataset, train_dataset]
        output_name = ['test', 'eval', 'train']
        ...
        for i, test_dataset in enumerate(test_datasets):
            ...
            output_test_file = os.path.join(
                training_args.output_dir, f"{output_name[i]}_results_{test_dataset.args.task_name}.txt"
            )
            if trainer.is_world_master():
            ...
            output_test_prob_file = os.path.join(
                training_args.output_dir, f"{output_name[i]}_results_{test_dataset.args.task_name}_prob.txt"
            )
            if trainer.is_world_master():
